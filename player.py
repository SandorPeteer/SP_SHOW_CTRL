...
from mpv_ipc import MpvIpcSession
...
def _no_console_subprocess_kwargs() -> dict:
    ...
    with _SHARED_MPV_OUTPUT_LOCK:
        ...
        if sess is None:
            sess = MpvIpcSession(
                ...
                ipc_verbose=bool(_is_file_logging_enabled(settings) or _is_debug_logging_enabled(settings)),
                app_log=_append_app_log,
                swallow_exc=_mpv_swallow,
                no_console_subprocess_kwargs=_no_console_subprocess_kwargs,
            )
...
class App(TkinterDnD.Tk if HAS_DND else tk.Tk):
    ...
    def _request_waveform_generate(self, deck: str, cue: Cue) -> None:
        ...
        def _worker():
            ...
            try:
                ...
                result = subprocess.run(cmd, capture_output=True, timeout=10, **_no_console_subprocess_kwargs())
    ...
    def _request_loudness_analysis(self, cue: Cue, *, force: bool = False, track_progress: bool = False) -> None:
        ...
        def _worker() -> None:
            ...
            try:
                ...
                res = subprocess.run(cmd, capture_output=True, text=True, timeout=float(timeout_sec), **_no_console_subprocess_kwargs())
        ...
        def _worker() -> None:
            ...
            try:
                ...
                sess = MpvIpcSession(
                    ...
                    second_screen_top=int(getattr(self.settings, "second_screen_top", 0)),
                    fullscreen=False,
                    ipc_verbose=bool(_is_file_logging_enabled(self.settings) or _is_debug_logging_enabled(self.settings)),
                    app_log=_append_app_log,
                    swallow_exc=_mpv_swallow,
                    no_console_subprocess_kwargs=_no_console_subprocess_kwargs,
                )
    ...
    def _install_mpv_prompt(self) -> None:
            ...
            def _import_mpv_windows() -> None:
                try:
                    chosen = filedialog.askopenfilename(
                        ...
                        filetypes=[("mpv.exe", "mpv.exe"), ("Executables", "*.exe"), ("All files", "*.*")],
                    )
                except Exception:
                    chosen = ""
                if not chosen:
        ...
        try:
            messagebox.showinfo("Install mpv", "mpv is not installed. I will open the installation page.", parent=self)
        except Exception:
            pass
        try:
            webbrowser.open(install_url)
